...
class Robot():
    ...
    def __init__(self, pos, vel, avel, controller, n, screen, cam_screen, obstacles, size=(10, 10), 
        ...
        self.sensors.append(IR_Sensor(tuple(map(operator.add, self.rect.center,
                                                (self.rect.height / 2 * math.cos(self.hangulation),
                                                 -self.rect.height / 2 * math.sin(self.hangulation)))),
                                      0, self, self.screen, self.obstacles, 20))
    ...
    def draw(self):
        ...
        irRotationLabel = font.render("IR Rotation: "+str(self.irRotationalDifference), 1, colors['white'])
        self.screen.blit(irRotationLabel, (500, 15 * self.n))
        translationLabel = font.render("Translation: " + str(self.forcedTranslationalDifference), 1, colors['white'])
        self.screen.blit(translationLabel, (300, 15 * self.n))
    ...
    def update(self):
        ...
        #gaussian distribution so that robots do not continually collide into each other
            ...
            if(self.sensors[0].obstacleDetected):# and self.sensors[1].obstacleDetected):
                ...
                print("left: rotation set:" + str(self.irRotationalDifference) + " translation set:" + str(self.forcedTranslationalDifference))
            elif (self.sensors[1].obstacleDetected):
                self.irRotationalDifference = random.gauss(25, 5) * (math.pi / 180)
                self.forcedTranslationalDifference = random.gauss(40, 10)
                print("mid: rotation set:" + str(self.irRotationalDifference) + " translation set:" + str(self.forcedTranslationalDifference))
            elif (self.sensors[2].obstacleDetected):# and self.sensors[1].obstacleDetected):
                self.irRotationalDifference = random.gauss(25, 5) * (math.pi / 180)
                self.forcedTranslationalDifference = random.gauss(40, 10)
                print("right: rotation set:" + str(self.irRotationalDifference) + " translation set:" + str(self.forcedTranslationalDifference))
