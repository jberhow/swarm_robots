...
while True:
  ...
  #print("M Current: {0:+.3f}, {1:+.3f}, {2:+.3f}".format(m_x, m_y, m_z))
 ...
 # else:
  if first_time:
    ...
    if abs(g_z) > 0.5:
        heading -= (prev_gz + g_z)*(0.5)*(time.clock() - prev_time)*22.5
    if abs(g_x) > 0.5:    
    ...
    if heading_mag < 0:
      heading_mag += 360.0
    heading = heading*0.95 + heading_mag*0.05 
  ...
  print("X Current: {0:+.3f}, {1:+.3f}, {2:+.3f} G Current: {3:+.3f}, {4:+.3f}, {5:+.3f} M Current: {6:+.3f}, {7:+.3f}, {8:+.3f} Pitch: {9:+.1f}  Roll: {10:+.1f}  Heading: {11:+.1f}".format(a_x, a_y, a_z, g_x, g_y, g_z, m_x, m_y, m_z, pitch, roll, heading))
